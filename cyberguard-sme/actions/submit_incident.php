...
require_login();
if ($_SERVER['REQUEST_METHOD'] !== 'POST') { header('Location: ../report_incident.php'); exit; }

$title = trim($_POST['title'] ?? '');
$desc = trim($_POST['description'] ?? '');
$severity = in_array($_POST['severity'] ?? 'medium', ['low','medium','high']) ? $_POST['severity'] : 'medium';

if (!$title) { header('Location: ../report_incident.php?error=Missing+title'); exit; }
...
header('Location: ../tracker.php?msg=incident+created');
exit;
